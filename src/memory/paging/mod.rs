...
impl Page {
    pub fn containing_address(addr: VirtualAddress) -> Page {
        assert!(addr < 0x0000_8000_0000_0000 || addr >= 0xffff_8000_0000_0000, "invalid address: 0x{:x}", addr);
        Page {
...
impl ActivePageTable {
        ...
        where F: FnOnce(&mut Mapper)
    {
...

        {
            ...
            // map `temp_page` to the current p4 table
...
{
    let mut temp_page = TemporaryPage::new(Page {
        number: 0xcafebabe,
    }, allocator);
    ...
    active_table.with(&mut new_table, &mut temp_page, |mapper| {
        ...
        let multiboot_end = Frame::containing_address(boot_info.end_address());
